...
const Paggination = ({ allEvents, limit, setPageNo, pageNo }) => {
    const intl = useIntl();
    ...
    return (
        <div className="pagination-festum">
            <p className="show-text">{intl.formatMessage({ id: "SHOWING" })} {(allEvents?.page - 1) * limit + 1} {intl.formatMessage({ id: "TO" })} {Math.min((allEvents?.page) * limit, allEvents?.totalDocs)} {intl.formatMessage({ id: "OF" })} {allEvents?.totalDocs} {intl.formatMessage({ id: "EVENTS" })}</p>
            <ul className="">
                <li>
