...
const EventPopUpUploadPhoto = ({ handleClose }) => {
	const intl = useIntl();
	return (
		//  <!-- Upload Photo  -->
					...
					<div className="bg-brightGray p-12">
						...
						<form className="py-7 space-y-5">
							<div className="upload-holder">
								<h6 className="text-sm font-bold text-quicksilver">{intl.formatMessage({ id: "SELECT PHOTO" })} <span className="text-10">{intl.formatMessage({ id: "5 IMAGES" })} ({intl.formatMessage({ id: "UP TO 3MB" })} / {intl.formatMessage({ id: "IMAGE" })})</span></h6>
