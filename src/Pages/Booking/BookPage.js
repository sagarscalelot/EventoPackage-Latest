...
const BookPage = ({ booking, limit, setPageNo, pageNo }) => {

    const intl = useIntl();
    // console.log("page : ", booking);
    ...
    const paginationList = paginator(Math.ceil(booking.totalPages), booking?.page, 7);
    return (
        <div className="pagination-festum">
            <p className="show-text">{intl.formatMessage({ id: "SHOWING" })} {(booking?.page - 1) * limit + 1} {intl.formatMessage({ id: "TO" })} {Math.min((booking?.page) * limit, booking?.totalDocs)} {intl.formatMessage({ id: "OF" })} {booking?.totalDocs} {intl.formatMessage({ id: "BOOKING" })}</p>
            <ul className="">
                <li>
                    <button className="btns" style={booking?.page < 2 ? { cursor: "not-allowed" } : {}} disabled={booking.page > 1 ? false : true} onClick={() => setPageNo(current => current - 1)}>
                ...
                {paginationList.map((e, i) => (
                    <li key={i}>
                        <button className={booking?.page === e ? "btns active" : "btns"} style={e != 0 ? {} : { cursor: "not-allowed" }} onClick={() => e != 0 && setPageNo(e)} disabled={e != 0 ? false : true} >
                            <span>{e != 0 ? e : "..."}</span>
                        </button>
                    </li>
                ))}
                ...
                <li>
                    <button className="btns" style={booking?.page === booking?.totalPages ? { cursor: "not-allowed" } : {}} disabled={booking?.page === booking?.totalPages ? true : false} onClick={() => setPageNo(current => current + 1)}>
